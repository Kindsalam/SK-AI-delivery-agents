...
- **Status:** default Backlog. No custom workflow change required.
- **Label:** `agent-drafted`. The sprint-planning view filters this label out via a JQL clause, so an agent-drafted story cannot be pulled into a sprint by accident. The PO removes the label when they accept the story. That is the human review signal.
...
- **Label `agent-drafted`** on every story it creates. The sprint-planning view filters this label out so sprint planning cannot pull the story by accident. PO removes the label when they accept the story.
- **Default Backlog status** so the story sits where new stories sit. No custom workflow change is required.
...
- **No `Sprint` assignment.** Even if capacity exists. The PO promotes by removing the label. Sprint planning then pulls the story like any other.
